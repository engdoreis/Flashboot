/*--------------------------------------------------------------------------------------------------
  qwInvertBits
----------------------------------------------------------------------------------------------------
  Invert the lower N bits of a variable (up to 64-bits).
--------------------------------------------------------------------------------------------------*/
#include "crc.h"
#include <stdint.h>
...
unsigned int crc32(const void *buf, int len, unsigned int init) {
  ...
  while (len--) {
    crc = (crc << 8) ^ crc32_table[((crc >> 24) ^ *((char *)buf)) & 255];
    buf++;
  }
  return crc;
}
...
uint16_t CRC_ITU(const uint8_t *data, uint32_t len) {
  uint32_t crc = 0xffff;
  uint32_t i;

  for (i = 0; i < len; i++) {
    uint8_t b = data[i];
    crc = ((crc >> 8) & 0x0FFFFFFF) ^ table[(crc ^ b) & 0xff];
  }
